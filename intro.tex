...
\begin{tikzpicture}[node distance = 2cm, auto]
  \node [block] (stars) {stars};
  \node [cloud, above of=stars] (name) {\underline{name}};
  \node [cloud, above left of=stars] (DoB) {\underline{DoB}};
  \node [cloud, left of=stars] (other) {\ldots};
  \node [decision, right of=stars] (contracts) {contracts};
  \node [cloud, above of=contracts] (salary) {salary};
  \node [block, right =1.5cm of contracts] (movies) {movies};
  \node [cloud, above of=movies] (title) {\underline{title}};
  \node [block, below of=contracts] (studios) {studios};
  \node [cloud, left of=studios] (room) {\underline{room}};
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance = 2cm, auto]
    \node [block] (car) {car};
    \node [cloud, above of=car] (make) {make};
    \node [cloud, above left of=car] (model) {model};
    \node [cloud, above right of=car] (color) {color};
    \node [cloud, right of=car] (year) {year};
    \path [line] (car) -- (make);
    \path [line] (car) -- (model);
    \path [line] (car) -- (color);
    \path [line] (car) -- (year);
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [cloud, above right of=car] (color) {color};
    \node [cloud, right of=car] (year) {year};
    \node [decision2, left=2cm of car] (issues) {Issues};
    \node [block, left=2cm of issues] (dealer) {dealer};
    \node [cloud, above of=issues] (VIN) {VIN};
    \path [line] (car) -- (make);
    \path [line] (car) -- (model);
    \path [line] (car) -- (color);
    \path [line] (car) -- (year);
...
\begin{figure}[H]
  ...
  \caption{Example ER Diagram}
\begin{tikzpicture}[node distance = 2cm, auto]
  \node [block] (movies) {movies};
  \node [decision, right of=movies] (owns) {owns};
  \node [block, right of = owns] (studio) {studio};
  \node [decision, right of=studio] (runs) {runs};
  \node [block, right of = runs] (president) {president};
